...
Please note that it will take a while to install the most recent version of Python on your computer. Once it's finished, type in `python` at your Command Line. Despite installing the most recent version of Python on your machine, your Terminal still likes using the old version. Teach it that you will tell it what to do by adding `export PATH="/usr/local/opt/python/libexec/bin:$PATH"` to your bash profile:
...
Next, install the LTS (long-term support) version of [Node](https://nodejs.org/en/about/releases/). Currently, it's version 16:
```sh
$ nvm install 16
```
...
You'll want to get your gitconfig setup to recognize your github credentials:
